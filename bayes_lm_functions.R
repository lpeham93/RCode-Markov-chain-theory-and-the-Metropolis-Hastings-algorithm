...
do.mh.posterior <- function(ntot, nburn, init, prior = "flat", prior_par, 
  ...
  for (i in 2:ntot){
    ...
    beta.prop <- rnorm(1, beta.save[i-1, ], prop_sd) #proposal for beta
    #beta.prop <- rmvnorm(1, beta.save[i-1, ], diag(prop_sd, k)) #proposal for beta, beta k-dimensional
  ...
  if (tuning == TRUE){return(acc_rate = acceptance/(ntot-nburn))}
