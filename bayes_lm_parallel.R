...
#Flat prior
tune.flat <- mclapply(as.list(seq(0.01, 0.2, length.out = 10)), 
...
#Normal prior
tune.normal <- mclapply(as.list(seq(0.01, 0.2, length.out = 10)), 
                        FUN = function(x) do.mh.posterior(prop_sd = x, ntot = 2000, 
                                                          nburn = 1000, init = 10, prior = "normal", 
                                                          prior_par = c(3,1), tuning = TRUE))
names(tune.normal) <- seq(0.01, 0.2, length.out = 10)

## -> set proposol sd to 0.05 which gives an acceptance of ~25%

#3)
#Sample posterior for different initialisation values
# Flat prior -------------------------------------------------------
results.flat <- mclapply(as.list(seq(-25, 25, by=5)), FUN = do.mh.posterior, 
...
mean(accept_rates)
beta.hats <- sapply(results.flat, FUN = function(x){apply(x, 2, mean)})
mean(beta.hats)
...
# Normal prior -----------------------------------------------------
results.normal <- mclapply(as.list(seq(-25, 25, by=5)), FUN = do.mh.posterior, 
...
mean(accept_rates)
beta.hats <- sapply(results.normal, FUN = function(x){apply(x, 2, mean)})
mean(beta.hats)
